...
tcell_clones <- read.table("../data/nsclc/vdj_v1_hs_nsclc_t_all_contig_annotations.csv", sep = ",", header = TRUE, stringsAsFactors = FALSE)
tcell_clones %>% filter(is_cell == "True"  & raw_clonotype_id != "None") %>%
  ...
  mutate(n = n()) %>% filter(n >= 3)  %>% arrange(raw_clonotype_id) -> filtered_t
...
af2 <- data.matrix(computeAFMutMatrix(SE[,as.character(filtered_t$barcode)]))
clones <- filtered_t$raw_clonotype_id
af2 <- af2[rowMeans(af2) > 0.0001,]

lapply((unique(clones)), function(clone){
  message(clone)
